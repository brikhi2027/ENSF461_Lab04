...
void policy_LT(int slice)
{
    ...
    LotLL = (struct jobPair*)malloc(sizeof(struct jobPair));
    LotLL->datum = temp;
    LotLL-> next = NULL;
    struct jobPair* JPPointer = LotLL;

    int tickets_total = temp->tickets;
    temp = temp->next;

    while (temp != NULL){
        tickets_total += temp->tickets;


        temp = temp->next;
    }
    int winning_ticket = rand() % tickets_total;
